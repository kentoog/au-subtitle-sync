...
from PyQt6.QtWidgets import (QApplication, QWidget, QVBoxLayout, QHBoxLayout, 
                             QLabel, QPushButton, QFileDialog, QSlider, QTextEdit, QColorDialog, QFrame, QStyle, QStyleOption, QMessageBox)
...
class DetachableLyricWindow(QFrame):
    def __init__(self):
        ...
        self.text_editor.setHorizontalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAsNeeded)
...
class LyricMasterControl(QWidget):
    def __init__(self):
        ...
        self.tile_mode = self.settings.value("tile_mode", False, type=bool)
        self.tile_button_locked = False
        self.loopmidi_process = None
        
        self.port_check_timer = QTimer()
        self.port_check_timer.timeout.connect(self.check_midi_port)
        ...
        try:
            ...
            self.loopmidi_process = subprocess.Popen(
                [r"C:\Program Files (x86)\Tobias Erichsen\loopMIDI\loopMIDI.exe"],
                startupinfo=startupinfo
            )
        except Exception as e:
            QMessageBox.critical(self, "错误", f"无法启动 loopMIDI:\n{str(e)}")
    ...
    def closeEvent(self, event):
        ...
        for proc in psutil.process_iter(['name']):
            if proc.info['name'] and 'Loopback MIDI communication' in proc.info['name']:
                try:
                    proc.terminate()
                    proc.wait(1000)
                except:
    ...
    def init_ui(self):
        ...
        self.btn_tile = QPushButton("平铺")
        self.btn_tile.setCheckable(True)
        self.btn_tile.setChecked(self.tile_mode)
        if self.tile_mode:
            self.btn_tile.setStyleSheet("background: #5a5; border: 1px solid #444; padding: 4px; font-size: 11px;")
        else:
            self.btn_tile.setStyleSheet(btn_style)
        self.btn_tile.clicked.connect(self.toggle_tile_mode)
        self.btn_midi = QPushButton("启动端口程序")
        self.btn_midi.setStyleSheet(btn_style)
        self.btn_midi.clicked.connect(self.launch_loopmidi)
        util_row.addWidget(self.btn_tile)
        util_row.addWidget(self.btn_midi)
    ...
    def toggle_tile_mode(self):
        if self.tile_button_locked:
            self.btn_tile.blockSignals(True)
            self.btn_tile.setChecked(self.tile_mode)
            self.btn_tile.blockSignals(False)
            return
        
        self.tile_button_locked = True
        self.btn_tile.setEnabled(False)
        
        self.tile_mode = self.btn_tile.isChecked()
        self.lyric_win.tile_mode = self.tile_mode
        self.settings.setValue("tile_mode", self.tile_mode)
        if self.tile_mode:
            self.btn_tile.setStyleSheet("background: #5a5; border: 1px solid #444; padding: 4px; font-size: 11px;")
        else:
            self.btn_tile.setStyleSheet("background: #333; border: 1px solid #444; padding: 4px; font-size: 11px;")
        self.refresh_text_display()
        if self.current_idx != -1:
            self.update_highlight(self.last_update_time, force=True)
        
        QTimer.singleShot(1000, self.unlock_tile_button)
    
    def unlock_tile_button(self):
        self.tile_button_locked = False
        self.btn_tile.setEnabled(True)

    def launch_loopmidi(self):
        try:
            if self.loopmidi_process is None or self.loopmidi_process.poll() is not None:
                startupinfo = subprocess.STARTUPINFO()
                startupinfo.dwFlags |= subprocess.STARTF_USESHOWWINDOW
                startupinfo.wShowWindow = 2
                self.loopmidi_process = subprocess.Popen(
    ...
    def refresh_text_display(self):
        ...
        if self.tile_mode:
            # 平铺模式：用空格分隔，在看板宽度范围内自动换行
            self.lyric_win.text_editor.setLineWrapMode(QTextEdit.LineWrapMode.WidgetWidth)
            all_text = []
            for _, txt in self.lyrics:
                all_text.append(txt)
            cursor.insertText("  ".join(all_text), fmt_norm)
        else:
            # 普通模式：换行显示
            self.lyric_win.text_editor.setLineWrapMode(QTextEdit.LineWrapMode.WidgetWidth)
            for _, txt in self.lyrics:
        ...
        if idx != -1 and (idx != self.current_idx or force):
            ...
            if self.current_idx != -1 and self.current_idx < len(self.lyrics) and self.current_idx != idx:
                if self.tile_mode:
                    ...
                    for i in range(self.current_idx):
                        pos += len(self.lyrics[i][1]) + 2
                    cursor.setPosition(pos)
                    cursor.setPosition(pos + len(self.lyrics[self.current_idx][1]), QTextCursor.MoveMode.KeepAnchor)
                    fmt = QTextCharFormat()
            ...
            if self.tile_mode:
                ...
                for i in range(idx):
                    pos += len(self.lyrics[i][1]) + 2
                cursor.setPosition(pos)
                cursor.setPosition(pos + len(self.lyrics[idx][1]), QTextCursor.MoveMode.KeepAnchor)
    ...
    def check_midi_port(self):
        ...
        try:
            ...
            port_keyword = 'AU_MTC'
            port_name = next((n for n in names if port_keyword in n), None)
            if port_name and not self.thread.isRunning():
                self.thread = MidiWorker()
                self.thread.raw_data_signal.connect(self.process_time)
                self.thread.status_signal.connect(self.update_status)
                self.thread.start()
